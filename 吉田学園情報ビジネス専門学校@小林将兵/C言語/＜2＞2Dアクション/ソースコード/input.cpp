...
void UpdateKeyboard(void)
{
	...
	if(SUCCEEDED(g_pDevKeyboard->GetDeviceState(sizeof(aKeyState),&aKeyState[0])))
	{
		...
		{
			g_aKeyStateTrigger[nCntKey] = (g_aKeyState[nCntKey] ^ aKeyState[nCntKey])& aKeyState[nCntKey];  // [ ^ ]排他的論理和　↓
			g_aKeyState[nCntKey] = aKeyState[nCntKey]; //キーボードの入力情報保存	↑　　入れ替える　入れ替えしないと連射のままになる
		}
	}
	else
	{
