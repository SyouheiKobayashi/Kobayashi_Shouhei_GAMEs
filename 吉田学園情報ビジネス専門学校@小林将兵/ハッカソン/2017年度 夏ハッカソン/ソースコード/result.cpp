...
void UpdateRESULT(void)
{
	...
	if (GetKeyboardTrigger(DIK_RETURN) == false)
	{
		...
		g_pVtxBuffRESULT->Lock(0, 0, (void**)&pVtx, 0);
		//タイトルカラー
		pVtx[4].col = D3DCOLOR_RGBA(g_aResult + 250, g_aResult - 210, g_aResult - 0, 255);
		pVtx[5].col = D3DCOLOR_RGBA(g_aResult + 260, g_aResult - 220, g_aResult - 0, 255);
		pVtx[6].col = D3DCOLOR_RGBA(g_aResult + 270, g_aResult - 230, g_aResult - 0, 255);
		pVtx[7].col = D3DCOLOR_RGBA(g_aResult + 280, g_aResult - 240, g_aResult - 0, 255);
		//頂点バッファアンロックする
		g_pVtxBuffRESULT->Unlock();
		//頂点バッファをロックし、頂点データへのポインタ
		g_pVtxBuffRESULT->Lock(0, 0, (void**)&pVtx, 0);
		pVtx[8].col = D3DCOLOR_RGBA(g_aResult * 2, 0, 0, 255);
		pVtx[9].col = D3DCOLOR_RGBA(g_aResult * 2, 0, 0, 255);
		pVtx[10].col = D3DCOLOR_RGBA(g_aResult * 2, 0, 0, 255);
		pVtx[11].col = D3DCOLOR_RGBA(g_aResult * 2, 0, 0, 255);
		//頂点バッファアンロックする
		g_pVtxBuffRESULT->Unlock();
	}
}
